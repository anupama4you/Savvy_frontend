...
const validate = (quote, messages, param) => {
    ...
    if (quote.monthlyFee != settings.Monthly_Fee__c) {
        warningList.push({
            field: "monthlyFee",
    ...
    if (
      quote.creditScore === undefined || 
      quote.creditScore === null || 
      quote.creditScore.length === 0
    ) {
      errorList.push({
        field: "creditScore",
        message: `Please select Credit Score.`
      });
    } else if (minLoan < 7500 || minLoan > quote.maxLoan) {
        warningList.push({
            field: "price",
    ...
    if (quote.dof > quote.maxDof) {
        warningList.push({
            field: "dof",
            message: `DOF should not be greater than ${quote.maxDof}`,
        });
    }
    ...
    console.log(
      "🚀 ~ file: quoteValidations.js ~ line 131 ~ validate ~ quote.clientRate",
      quote.clientRate
    );
