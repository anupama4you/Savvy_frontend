      ...
      INSURANCE_FIELDS.forEach((value, key) => {
        ...
        if (quoteData.data[`${value}`] && key) {
          if (
            ...
            if (key.includes("warranty") || key.includes("Integrity")) {
...
const mapLWCToSObject = (
  ...
  console.log(
    "🚀 ~ file: quoteCommons.js ~ line 544 ~ includeInsurance",
    includeInsurance
  );
  ...
  // data
  console.log(
    `@@mapLWCToSObject-quote form : ${JSON.stringify(quoteForm, null, 2)}`
  );
  console.log(
    `@@mapLWCToSObject-fields : ${JSON.stringify(
      QUOTE_FIELDS_MAPPING,
      null,
      2
    )}`
  ...
  try {
    for (const [key, value] of FULL_FIELDS_MAPPING) {
      ...
      if (includeInsurance) {
        ...
        console.log(`[${key}]`, value, `: `, obj.data[value]);
  ...
  // Check values
  console.log(`@@obj.data["DOF__c"]:`, obj.data["DOF__c"]);
  if (obj.data["DOF__c"] === undefined || obj.data["DOF__c"] === null) {
    obj.data["DOF__c"] = 0.0;
  }
