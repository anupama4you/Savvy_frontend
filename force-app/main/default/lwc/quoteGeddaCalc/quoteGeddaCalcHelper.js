  ...
  new Promise((resolve, reject) => {
    ...
    if (res.messages && res.messages.errors.length > 0) {
      ...
      const p = {
        ...
        totalAmount: (quote.price - QuoteCommons.calcNetDeposit(quote)),
...
const calcOptions = {
  ...
  terms1: CommonOptions.terms(12, 60),
  ...
  new Promise((resolve, reject) => {
    ...
    getQuotingData({
      ...
      .then((quoteData) => {
        ...
        // Display None option of Customer grading
        if (!data.customerGrading) {
          data.customerGrading = "";
        }
        resolve(data);
      })
...
const calculateApplicationFee = (quote) => {
  let appFee = 0.0;
  console.log('blaah@@', quote.customerProfile, '---', quote.customerGrading)
...
const calculateRealTimeNaf = (quote) => {
  let naf = 0.0;
  naf += (quote.price - QuoteCommons.calcNetDeposit(quote)) + QuoteCommons.calcTotalInsuranceType(quote, "Q");
  return naf;
};
...
const renderTerms = (quote) => {
  try {
    ...
    if (quote.customerProfile === "Asset Finance") {
      ...
      defaultVal = naf > 5000 ? 36 : 24;
    } else if (quote.customerProfile === "Mini Moto Plus") {
      terms = getTerms(36, 36, 12);
      defaultVal = 36;
    }

    return { 'options': terms, 'default': defaultVal };
  } catch (error) {
    console.error(error);
  }
};
...
export const CalHelper = {
  ...
  TERM_OPTIONS_FIELDS: TERM_OPTIONS_FIELDS,
