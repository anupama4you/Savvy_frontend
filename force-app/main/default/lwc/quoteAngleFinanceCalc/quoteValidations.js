...
const validate = (quote, messages, param) => {
  ...
  if (quote.baseRate === undefined || quote.baseRate === null) {
    errorList.push({
      field: "baseRate",
      message: "Loan is not available for this age and vehicle"
    });
  }
  ...
  const p = quote.price*0.2;
  console.log(`param:`, p, JSON.stringify(param, null, 2));

  if("N" === quote.propertyOwner && param.netDeposit < p) {
    warningList.push({
      field: "deposit",
  ...
  warningList.push({
    field: "price",
    message: `Review Asset amounts available from below chart`
  });
