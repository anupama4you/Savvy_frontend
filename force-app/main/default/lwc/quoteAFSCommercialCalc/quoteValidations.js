...
const validate = (quote, messages, nafVal, afsRates) => {
  ...
  if (quote.clientRate == null || quote.clientRate === 0.0) {
    ...
    if (afsRates && afsRates.length > 0) {
      let minRate = 0.0;
      let maxRate = 0.0;
      let afsRateSize = afsRates.length - 1;
      
      if (quote.gst === "Not Registered") {
        minRate = afsRates[3+1].rate !== null ? afsRates[3+1].rate : 0.0;
        maxRate =
          afsRates[afsRateSize].rate != null ? afsRates[afsRateSize].rate : 0.0;
      } else if (quote.carAge === "New - 6 years old") {
        if (quote.residency === "Home Buyer") {
          minRate = afsRates[0].rate !== null ? afsRates[0].rate : 0.0;
        } else {
          minRate = afsRates[4].rate !== null ? afsRates[4].rate : 0.0;
        }
        maxRate =
          afsRates[afsRateSize - 3].rate != null
            ? afsRates[afsRateSize - 3].rate
            : 0.0;
      } else {
        if (quote.residency === "Home Buyer") {
          minRate = afsRates[0].rate !== null ? afsRates[0].rate : 0.0;
        ...
        maxRate =
          afsRates[afsRateSize].rate != null
            ? afsRates[afsRateSize].rate
            : 0.0;
      }
      ...
      if (quote.clientRate < minRate) {
        errorList.push({
          field: "clientRate",
          message: `The minimum Client Rate for ${quote.carAge} is ${minRate}%, please check your value and try again.`
        });
      }
      ...
      if (quote.clientRate > maxRate) {
        errorList.push({
          field: "clientRate",
          message: `The maximum Client Rate for ${quote.carAge} is ${maxRate}%, please check your value and try again.`
        });
      }
    }
  }
