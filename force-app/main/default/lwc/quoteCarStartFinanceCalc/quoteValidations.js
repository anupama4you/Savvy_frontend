...
const validate = (quote, messages) => {
  ...
  if (
    quote.loanTypeDetail === undefined ||
    quote.loanTypeDetail === null ||
    quote.loanTypeDetail.length === 0
  ) {
    errorList.push({
    ...
    if (quote.commission > 500) {
      warningList.push({
        field: "commission",
        message: `Bonus Commission should not be greater than $500.00`
      });
    }
    if ("Mid-Prime Finance Only" === quote.loanTypeDetail) {
      ...
      if (
        quote.clientRate < quote.baseRate ||
        quote.clientRate > quote.maxRate
      ) {
        warningList.push({
          field: "clientRate",
      ...
      if (
        "Sub-Prime Finance Only" === quote.loanTypeDetail ||
        "Sub-Prime Lite (pensioners) Finance Only" === quote.loanTypeDetail
      ) {
        if (
          quote.riskFee < quote.riskFeeRate ||
          quote.riskFee > quote.maxRiskFee
        ) {
          warningList.push({
            field: "riskFee",
  ...
  if (quote.commission < 0.0) {
    errorList.push({
      field: "commission",
      message: `Bonus Commission should not be below Zero.`
    });
  }
  ...
  if (!quote.loanPurpose) {
    warningList.push({
      field: "loanPurpose",
      message: "Loan Purpose could be necessary"
    });
  }
