...
export default class QuoteCarStartFinanceCalc extends LightningElement {
  ...
  get isValidMaxRiskFee() {
    return this.quoteForm.maxRiskFee && this.quoteForm.maxRiskFee != 0
      ? true
      : false;
  }

  get isValidMaxDof() {
    return this.quoteForm.maxDof &&
      this.quoteForm.maxDof != 0 &&
  ...
  get netRealtimeNaf() {
    this.quoteForm["realtimeNaf"] =
      CalHelper.getNetRealtimeNaf(this.quoteForm);
    ...
    CalHelper.baseRates(this.quoteForm)
      .then((data) => {
        ...
        if (this.quoteForm.loanTypeDetail) {
          const result = this.tableRates.filter(
            (data) => data.Type_of_Finance__c == this.quoteForm.loanTypeDetail
          );
          this.quoteForm.maxDof = result[0].Dof_Max__c
            ? result[0].Dof_Max__c
            : null;
