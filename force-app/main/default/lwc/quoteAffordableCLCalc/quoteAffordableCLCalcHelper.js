...
const getNaf = (quote) => {
  ...
  const insProds = QuoteCommons.calcTotalInsuranceType(quote);
  console.log("@@getNaf:", r, insProds)
  r += insProds;
  return r;
}

const calcRiskFee = (quote) => {
  // console.log("🚀 ~ file: quoteAffordableCLCalcHelper.js ~ line 295 ~ calcRiskFee ~ quote", JSON.stringify(quote, null, 2));
  let r = 0;
  let myRate =
    quote.clientRate > 0
      ? quote.clientRate
  ...
  const loanAmountBase = getNaf(quote);
  ...
  console.log(
    ...
    myRate,
    quote.term,
    affordableFees.Risk_Fee_Interest_Rate_Of_NAF__c
  );
  ...
  if (
    affordableFees &&
    affordableFees.Risk_Fee_Interest_Rate_Of_NAF__c > 0 &&
    quote.term > 0
  ) {
    r =
      (((myRate / 100) * affordableFees.Risk_Fee_Interest_Rate_Of_NAF__c) /
        100) *
      (quote.term / 12) *
      getNaf(quote);
  }
...
const getMyBaseRates = (quote) =>
  new Promise((resolve, reject) => {
    ...
    getBaseRates({
      ...
      .then((rates) => {
        let myRate = quote.clientRate > 0 ? quote.clientRate : rates.maxRate;
        // console.log(`@@SF:`, JSON.stringify(rates, null, 2), myRate, quote.clientRate);
