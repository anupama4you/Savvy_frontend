...
const calculatingLPI = (
  ...
  new Promise((resolve, reject) => {
    ...
    if (LPIType && LPIType.includes("Liberty") && LPIProduct) {
      ...
      if (quoteForm.riskFee > 0) {
        fields.push(["riskFee", "Risk_Fee__c"]);
      }
      if (quoteForm.eqfee > 0) {
        fields.push(["eqfee", "Risk_Fee__c"]);
      }
      if (quoteForm.commission > 0) {
        fields.push(["commission", "Extra_Value_1__c"]);
      }
      ...
      if (
        ...
        (
          ...
          quoteForm.name.includes('Westpac') 
        )
      ) {
        qt.data.DOF__c = 0.0;
      ...
      calculateLPI({
        ...
        .then((data) => {
          console.log(`@@LPI result:`, JSON.stringify(data, null, 2));
          resolve(data);
        })
        .catch((error) => {
          reject(error);
