...
let tableRateDataColumns = [
  ...
  { label: "Tier", fieldName: "Tier__c", type: "text", fixedWidth: 100 },
  { label: "Minimum Equifax Score", fieldName: "Min_Equifax_Score__c", type: "text", fixedWidth: 220 },
  { label: "Overs", fieldName: "Overs__c", type: "text" },
  { label: "New/Demo", fieldName: "Rate0__c", type: "text" },
...
const LENDER_QUOTING = "RateSetter";
      ...
      .then((quoteData) => {
        ...
        if (quoteData.rateSettings) {
          tableRatesData = quoteData.rateSettings[`${RATE_SETTING_NAMES[0]}`]
            .filter(rate => rate.Loan_Product__c === "Consumer");
          // tableRatesData = quoteData.rateSettings[`${RATE_SETTING_NAMES[0]}`];
...
const getMyBaseRates = (quote) =>
  new Promise((resolve, reject) => {
    ...
    getBaseRates({
      ...
      .then((rates) => {
        console.log('==> getMyBaseRates rates ', JSON.stringify(rates, null, 2));
        resolve(rates);
      })
      .catch((error) => reject(error));
  });
