...
import { CalHelper } from "./quoteMetroCalcHelper";
import LENDER_LOGO from "@salesforce/resourceUrl/MetroLogo";
...
export default class QuoteMetroCalc extends LightningElement {
  tableRatesCols = CalHelper.tableRateDataColumns;
  tableRateColsForPlanOptions = CalHelper.tableRateDataColumnsForPlanOptions;
  isBusy;
  isBaseRateBusy;
  isCalculated = false;
  tableRate1;
  ...
  connectedCallback() {
    ...
    CalHelper.load(this.recordId)
      .then((data) => {
        ...
        this.tableRate1 = this.tableRates["PassengerAndCommercialRates"];
        this.tableRate2 = this.tableRates["HeavyCommercialRates"];
        this.tableRate3 = this.tableRates["WhelledPlantEquipmentRates"];
        this.tableRate4 = this.tableRates["MetroPlanOption__c"];
      })
      .catch((error) => {
  ...
  get planOptions() {
    return CalHelper.options.planOptions;
  }

  get vehicleConditionsOptions() {
    return CalHelper.options.vehicleCondition;
  }

  get salesBacksPurchasedOptions() {
    return CalHelper.options.salesBacksPurchased;
  }
  ...
  get greenCarOptions() {
    return CalHelper.options.greenCar;
  }
  ...
  handleFieldChange(event) {
    ...
    fldName === "term"
      ...
      : (this.quoteForm[fldName] = v);
    if (fldName === "planOptions") {
      const rates = CalHelper.resetLenderSetting(this.quoteForm[fldName]);
      this.quoteForm.applicationFee = this.quoteForm.maxApplicationFee =
        rates.applicationFee;
      this.quoteForm.dof = this.quoteForm.maxDof = rates.dof;
    }
